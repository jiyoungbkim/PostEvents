...
<%	
	...
	String news_no = request.getParameter("t_qandNo");
	...
	ArrayList<Faq_DTO> arrF = dao.getFaqList(selValue,txtValue);
	...
	//*********page start***********/
		...
		if(arrF == null) total_count =0;
		else total_count = arrF.size(); 
	...
	<body>
		...
		<div class="notice">
			...
			<div class="faq-box">
				...
				%>
				<button class="accordion">▶ <%=arrF.get(k).getTitle()%></button>
				<div class="text">
					<p><%=arrF.get(k).getContent()%></p>
					<div class="list">
					<a href="faq_u.jsp?t_faqNo=<%=arrF.get(k).getFaq_no()%>">Modify</a>&nbsp;&nbsp;
					<a href="javascript:deleteFaq('<%=arrF.get(k).getFaq_no()%>')">Delete</a>
					</div>
				</div>
		...
		<div class="page-number">
				<%if(current_page != 1) { %>
				<a href="<%="faq_r.jsp?t_sel="+selValue+"&t_search="+txtValue+"&r_page="+p_page%>" class="icon"><i class="fas fa-arrow-circle-left fa-lg"></i></a>
				<% } %>
				<%
				...
				<a href="<%="faq_r.jsp?t_sel="+selValue+"&t_search="+txtValue+"&r_page="+n_page%>" class="icon"><i class="fas fa-arrow-circle-right fa-lg"></i></a>
				<%} %>
				<a href="faq_w.jsp" class="btn-write">글쓰기</a>
			</div>
