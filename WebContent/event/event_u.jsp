...
<%@ page import="common.CommonUtil,dao.Event_DAO,dto.Event_DTO"%>
<%
...
<html lang="ko">
	...
	<body>
		...
		<div class="notice">
			...
			<form name="write">
			<input name="t_event_no" type="hidden" value="<%=event_no%>">
					...
					<fieldset>
						...
						<table class="table">
							...
							<tr>
								<th><label for="date">작성일</label></th>
								<td colspan="2"><input type="text" name="r_date" id="date" class="date" value="<%=dtoE.getReg_date()%>" readonly></td>
							</tr>
							
							<tr>
								<th><label for="title">제목</label></th>
								<td colspan="2"><input type="text" name="title" id="title" class="title" value="<%=dtoE.getTitle()%>" placeholder="제목을 입력해주세요"></td>
							</tr>
							...
							<tr>
								...
								<td class="date">&nbsp; 이벤트 시작일  &nbsp;<input type="date" name="s_date" class="e_date" value="<%=dtoE.getReg_start() %>" placeholder="이벤트 접수 시작일을 입력해주세요">&nbsp;&nbsp;
								이벤트 종료일 &nbsp;<input type="date" name="e_date" class="e_date" value="<%=dtoE.getReg_end()%>" placeholder="이벤트 접수 종료일을 입력해주세요"></td>
							</tr>
							
							<tr>
								<th><label for="cont">내용</label></th>
								<td colspan="2"><textarea type="cont" name="cont" id="cont" class="cont" placeholder="내용을 입력해주세요"><%=dtoE.getContent()%></textarea>
							</tr>
		...
		<script>
			function eventcheck(reg_d) {
				...
				if(fm.s_date.value < fm.r_date.value){
					alert("시작일자는 작성일 이후로 설정해야 합니다");
					fm.s_date.focus();
					return ;
				}
