...
//*********page start***********/
	...
	if(arrE == null) total_count =0;
	else total_count = arrE.size(); 
	...
	function prize(emNo) {
		...
		<form name="notice">
					...
					%> 	
					 <tr>
						...
						<td><%if(reg_date >= CommonUtil.datetoInt(arrE.get(k).getReg_start()) && reg_date <= CommonUtil.datetoInt(arrE.get(k).getReg_end())) { %>
							<p style=background:lightblue>이벤트 진행중</p>
						<% } else { %> 
						<p style=background:pink>이벤트 종료</p> 
						<% } %>
						<td name="s_date"><%=arrE.get(k).getReg_start()%></td>
						<td name="e_date"><%=arrE.get(k).getReg_end()%></td>
						<td><%=arrE.get(k).getReg_id()%></td>					
						<td><%=arrE.get(k).getReg_date()%></td>
						<td><%=arrE.get(k).getHit()%></td>
			...
			<div class="page-number">
				<%if(current_page != 1) { %>
				<a href="<%="event_r.jsp?t_sel="+selValue+"&t_search="+txtValue+"&r_page="+p_page%>" class="icon"><i class="fas fa-arrow-circle-left fa-lg"></i></a>
				<% } %>
				<%
				...
				<a href="<%="event_r.jsp?t_sel="+selValue+"&t_search="+txtValue+"&r_page="+n_page%>" class="icon"><i class="fas fa-arrow-circle-right fa-lg"></i></a>
				<%} %>
				<%if(sessionId.equals("manager")) { %>
				
				<a href="event_w.jsp" class="btn-write">글쓰기</a>
				<% } %>
			</div>	
		</div>		
		<!--  footer start  -->
